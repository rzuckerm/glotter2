...
def test_add_test_mapping_when_project_type_not_in_projects(temp_dir_copy_glotter_yml):
    with pytest.raises(KeyError):
        get_settings().add_test_mapping("notarealprojectype", None)
...
def test_add_test_mapping_when_project_type_has_no_mappings(temp_dir_copy_glotter_yml):
    ...
    get_settings().add_test_mapping("baklava", test_func)
    assert test_func.__name__ in get_settings().get_test_mapping_name("baklava")
...
def test_add_test_mapping_when_project_type_already_has_mapping(
    ...
    get_settings().add_test_mapping("baklava", test_func)
    get_settings().add_test_mapping("baklava", test_func2)
    assert test_func.__name__ in get_settings().get_test_mapping_name("baklava")
    assert test_func2.__name__ in get_settings().get_test_mapping_name("baklava")


def test_get_test_mapping_name_when_project_type_not_found(temp_dir_copy_glotter_yml):
    assert get_settings().get_test_mapping_name("nonexistentproject") == []


def test_projects(temp_dir_copy_glotter_yml):
    assert get_settings().projects == {
...
def test_project_root(temp_dir_copy_glotter_yml):
    assert get_settings().project_root == temp_dir_copy_glotter_yml
...
def test_source_root(glotter_yml_filename, expected_source_root, temp_dir_chdir):
    ...
    assert get_settings().source_root == os.path.abspath(
...
def test_source_root_setter_empty_value(temp_dir_copy_glotter_yml):
    ...
    assert get_settings().source_root == temp_dir_copy_glotter_yml


def test_source_root_setter_value(temp_dir_copy_glotter_yml):
    value = os.path.join(temp_dir_copy_glotter_yml, "data")
    get_settings().source_root = value
    assert get_settings().source_root == value
...
def test_test_mappings(temp_dir_copy_glotter_yml):
    ...
    get_settings().add_test_mapping("baklava", test_func)
    get_settings().add_test_mapping("fileio", test_func2)
    get_settings().add_test_mapping("fileio", test_func3)
    get_settings().add_test_mapping("fibonacci", test_func4)

    assert get_settings().test_mappings == {
        "baklava": [test_func],
...
def test_verify_project_type(project_name, expected_result, temp_dir_copy_glotter_yml):
    assert get_settings().verify_project_type(project_name) == expected_result
